...
function calculateRisk(patient, sys, dia){
  ...
  console.log('age: ' + age)
  console.log('ldl: '+ patient.ldl)
  console.log('hdl: ' + patient.hdl)
  console.log('sys: '+ sys)
  console.log('dia: ' + dia)
  ...
  client.request("Observation?" + query, {
    ...
    function(ob) {
      ...
      for(i=0;i<smoke.length;i++){
        if('valueCodeableConcept' in smoke[i]){
          if('text' in smoke[i]['valueCodeableConcept']){
            if(smoke[i]['valueCodeableConcept']['text'] == 'Never smoker' || smoke[i]['valueCodeableConcept']['text'] == 'Unknown if ever smoked'){
              p.smoker = false;
            }
          }
        }
